...
What the numbers mean: structure extraction is exact; entry-point F1 tracks repository shape — gum's single-entry CLI is the best case, click's library-only layout is detected too (the deterministic package-root heuristic, Roadmap item 2, emits `src/click/__init__.py` as a `library_api` candidate; the LLM confirms it from the sampled `__init__.py`, raising the 0.40 confidence to 0.90 with an `import click` invocation), and flask's 0.67 is the documented cost of greedy recall (all three real entries found, three expected false positives re-ranked away). The hardest guarantee, 0% hallucination, holds on **all three** reports (flask 16/16, click 15/15, gum 18/18 on the 2026-08-27 snapshot). The 2026-08-27 rows re-measure after Roadmap item 3 (digest-metric deduction): reports now carry a deterministic **Verified Facts** annex — pipeline-computed numbers, never LLM-authored — and the judge rubric exempts claims matching it. Grounding medians went 3 → 5 (gum) and 3.5 → 5 (flask), correctness/usefulness moved with them, and the recurring "no file's content can prove this number" deductions are gone from the judge's comments; under the same rubric the pre-annex flask report still judges grounding 3 — the exemption is the annex, not a looser rubric (see [baseline.md](evals/results/baseline.md)).
...
4. **More gold cases** — Go monorepos, Node CLIs, Rust crates; each adds a row to the baseline and guards prompt regressions.
