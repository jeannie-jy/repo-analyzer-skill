...
6. ~~**Evaluation depth**~~ — **DONE (2026-08-30, part 1/2)**: the judge now scores every content section of the report (grounding + correctness per section) in addition to the overall five criteria — the rubric asks for a `sections` array first, then the flat scores; parsing is lenient (a missing or malformed array degrades to the old whole-report-only contract, so old judge output still parses). `eval --judge` prints a per-section summary (count, averages, names of sections scoring ≤ 2 on either criterion). Verified on a real zh click report: 13/13 sections scored with Chinese comments, avg grounding 4.46 / correctness 4.69 consistent with the overall 4/4/4/5,5, no weak sections. Remaining: multi-model judge ensemble (needs a second provider key).
