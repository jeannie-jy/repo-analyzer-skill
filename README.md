...
4. ~~**More gold cases**~~ — **DONE (2026-08-30)**: `sharkdp/fd` (Rust crate) + `11ty/eleventy` (Node SSG) added, five cases total. The run documented the Cargo blind spot (fd F1=0.00 — no `[[bin]]` detection) and closed an annex coverage gap it exposed (largest-files cap 8 → 15, matching the sampler tier, so every byte count the LLM can restate is listed; `RepoFacts.from_dict` added). The Cargo bin heuristic landed the same day — every `Cargo.toml` in the tree is parsed (workspace member crates declare their own bins): `[[bin]]` targets (path resolved relative to the manifest's directory, conf 0.95 in-tree), the default `src/main.rs` bin (conf 0.90), and the default `src/lib.rs` / explicit `[lib]` path as a `library_api` candidate under the same cli/http_server gate as the Python library root; the Makefile rules tightened along the way (bare presence no longer yields `build_entry`; `.PHONY` needs a run/dev/serve/test keyword) — fd F1 0.00 → 1.00 (`src/main.rs`, conf 0.95, `cargo run --bin fd`). eleventy F1=1.00 (package.json bin positive control); all older cases unchanged (regression guard held); judge N=3: fd 5/5/5/5,5, eleventy 5/4/4/5,5 with only scope-legal "implied" deductions.
5. ~~**Report language coverage**~~ — **DONE (2026-08-30)**: `REPORT_LANGUAGE=zh` is now end-to-end. The render layer localizes via a deterministic translation table (`report/labels.py` — 14 section titles, header/table/inline labels, the digest annex; `en` stays byte-identical), the report records its `language` key so report.json re-renders standalone, and the LLM gets an additive directive to write free-text in Chinese while pinning everything structural (the 13 top-level keys, enum values, paths, symbols, numbers — translating enums would fail schema validation). The "Verified Facts" anchor survives under zh (`已验证事实 (Verified Facts, pipeline-computed)`) so the judge rubric stays literally true, and the rubric now says reports may be English or Chinese. Verified end-to-end on a real zh click report: 14/14 zh section titles, 0 translated evidence paths, enums intact (`library_api`/`low`/`high`), schema validation clean, judge scored it 5/4/4/5,5 with Chinese comments.
6. ~~**Evaluation depth**~~ — **DONE (2026-08-30)**: two upgrades.
   **Per-section scoring**: the judge now scores every content section of the report (grounding + correctness per section) in addition to the overall five criteria — the rubric asks for a `sections` array first, then the flat scores; parsing is lenient (a missing or malformed array degrades to the old whole-report-only contract, so old judge output still parses). `eval --judge` prints a per-section summary (count, averages, names of sections scoring ≤ 2 on either criterion). Verified on a real zh click report: 13/13 sections scored with Chinese comments, avg grounding 4.46 / correctness 4.69 consistent with the overall 4/4/4/5,5, no weak sections.
   **Multi-model judge ensemble**: an optional second judge provider (`JUDGE_BASE_URL` / `JUDGE_API_KEY` / `JUDGE_MODEL` — base url and model fall back to the main LLM's, so a bare key suffices) turns `eval --judge` into an ensemble: every report is scored by both models, the flat score line becomes the **median** across models (even counts average the two middle values, the same `.5` convention baseline.md already uses), per-model scores stay visible in a `models` line and in the JSON, and per-section scores are re-medianed per section name. Unset JUDGE_API_KEY keeps the single-model contract byte-identical. Judge calls retry on parse failures (`MAX_JUDGE_RETRIES`) — reasoning models occasionally truncate output when the chain of thought eats the token budget, and one bad reply must not abort the eval run. Verified on the real zh click report with deepseek-v4-flash vs deepseek-v4-pro: both independently scored all 13 sections with no weak sections and identical avg correctness 4.77; the one disagreement — grounding 4 (flash) vs 3 (pro, stricter on evidence directness) — lands at the median 3.5, so the ensemble keeps both models' consensus and blunts either model's lone vote.
