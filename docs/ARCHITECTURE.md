...
| entrypoint 候选检测 | 脚本启发式 + LLM 排序解释 | 启发式可穷举（package.json bin、`__main__`、Dockerfile CMD、Makefile…），语义解释交给 LLM |
...
| evidence grounding | 确定性 citation 校验（路径是否真实存在于 tree） | Phase 8 |
